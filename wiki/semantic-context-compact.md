...
- `working_contexts` 是不可变快照表；`working_context_source_heads` 为每个 `(run_scope, workflow_id)` 维护单调 source revision，`working_context_source_clock(run_scope, workflow_id, revision)` 由源表触发器按 execution scope 维护，最新相同 fingerprint 复用旧 `context_id`，事实变化或 A→B→A 追加新快照；迟到旧 revision 不能成为 latest，同一 scope 的保存竞态会重试，其他 Workflow 的写入不会误触发 stale；dispatch 重新读取权威 Task 身份。
