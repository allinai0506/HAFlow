...
def replay_run(
    ...
    *,
    ...
    if definition is None:
        warnings.append("definition_not_fully_frozen")
    ...
    if definition is not None:
        try:
            snapshot = projects.freeze_run_definition(target_workflow, definition=definition)
        except ValueError:
            raise
    ...
    state_db.save_task(
        {
            ...
            "agent": source_task.get("agent") or "auto",
            "status": "pending",
            "goal": source_task.get("goal") or f"replay of {source_run_id}",
        },
        db_path=db_path,
    )
