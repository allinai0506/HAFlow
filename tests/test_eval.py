...
def test_record_and_get_eval_result_roundtrip(eval_db: Path):
    ...
    assert stored["scores"] == {"accuracy": 1.0}
...
def test_eval_record_is_idempotent_on_run_revision(eval_db: Path):
    ...
    first = eval_store.record_eval_result(
        "run-idem", revision=1, verdict="pass", db_path=eval_db
    )
...
def test_eval_malformed_json_degrades_to_none(eval_db: Path):
    ...
    stored = eval_store.record_eval_result(
        "run-malformed", revision=1, verdict="pass",
        scores={"ok": True}, db_path=eval_db,
    )
    conn = state_db.get_db_connection(eval_db)
    try:
        conn.execute(
            "UPDATE eval_results SET scores_json = 'not-json{' WHERE eval_id = ?",
            (stored["eval_id"],),
        )
        conn.commit()
    finally:
    ...
    assert fetched is not None
    # Malformed scores must degrade to None, never raise or return raw text.
    assert fetched["scores"] is None
    assert fetched["verdict"] == "pass"
...
def test_eval_null_semantics_preserved(eval_db: Path):
    ...
    stored = eval_store.record_eval_result(
        "run-null", revision=1, verdict=None, scores=None,
        evidence=None, db_path=eval_db,
    )
...
def test_delete_workflow_retains_eval_rows(eval_db: Path):
    ...
    eval_store.record_eval_result(
        "run-keep", revision=1, verdict="pass", db_path=eval_db
    )
...
def test_state_store_delegates_eval(eval_db: Path):
    ...
    stored = store.record_eval_result("run-store", revision=1, verdict="pass")
