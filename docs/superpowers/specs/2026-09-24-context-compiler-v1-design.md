...
- `working_context_source_heads` 保存每个 `(run_scope, workflow_id)` 的 source version 与单调 revision；`working_context_source_clock(run_scope, workflow_id, revision)` 由源表触发器维护并参与同一 execution scope 的保存竞态检查；
